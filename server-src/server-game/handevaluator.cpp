...
HandRankType HandEvaluator::evaluateHand(QVector<Card> hand){
    ...
    else if(isSINGLE_PAIR()) return HandRankType::SINGLE_PAIR;
...
//double pair still works
        ...
        {
            QList<Rank> pairs1, pairs2;

            Rank kickers1, kickers2;
            QVector<Card> hand_player1=player1->getHand();
            QVector<Card> hand_player2=player2->getHand();
            for (int i = 0; i <=4; ++i) {
                if (hand_player1[i] == hand_player1[i+1]){
                    pairs1.append(hand_player1[i].getRank());
                    i++;
                }else {
                    kickers1 = hand_player2[i].getRank();
                }
            }
            for (int i = 0; i <=4; ++i) {
                if (hand_player2[i] == hand_player2[i+1]) {
                    pairs2.append(hand_player2[i].getRank()) ;
                    i++;
                }
                else {
                    kickers2 = hand_player1[i].getRank();
                }
            }
            ...
            if (kickers1 > kickers2) return player1;
            if (kickers2 > kickers1) return player2;
            break;
        }
//single pair still works
        case HandRankType::SINGLE_PAIR:
        {
            ...
            for (int i = 0; i <=4; ++i) {
                if (hand_player1[i] == hand_player1[i+1]) {
                    pairRank1 = hand_player1[i].getRank();
                    i++;
                }
                else {
                    kickers1.append(hand_player1[i].getRank());
                }
            }
            for (int i = 0; i <=4; ++i) {
                if (hand_player2[i] == hand_player2[i+1]) {
                    pairRank2 = hand_player2[i].getRank();
                    i++;
                }
                else {
                    kickers2.append(hand_player2[i].getRank());
                }
            }
